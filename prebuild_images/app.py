...
def gfg():
    ...
    and redirects to root signin if invalid entry.
    """
	# getting input with name = fname in HTML form
    user = request.form.get("username")
	# getting input with name = lname in HTML form
    password = request.form.get("password")
    #get 2fa input if empty string return None
    twofa_code = request.form.get("2fa")
    print((user,password,twofa_code))
    kc_auth = KCAuth(
		KC_DOMAIN, KC_CLIENTREALM,
		KC_CLIENTID, KC_CLIENTSECRET
		)
    if twofa_code != "":
        token = kc_auth.get_token_2fa(user,password,twofa_code)
    else:
        token = kc_auth.get_token_basic(user,password,twofa_code)
    resp = kc_auth.token_introspect(token)
    if resp['active'] is False:
        return redirect("/", code=302)
